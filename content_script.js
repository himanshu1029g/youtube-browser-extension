...
waitForVideo((v) => {
  ...
  // FEATURE 2: Tell background when user plays/pauses
  v.addEventListener('play', () => {
    if (!isAlive() || pausedByUs) return;
  ...
  // FEATURE 1A: Cursor re-enters
  document.addEventListener('mouseenter', () => {
    if (!isAlive() || !v.paused || !pausedByUs) return;
    pausedByUs = false;
    v.play().catch(() => {});
    safeMsg({ type: 'VIDEO_PLAYING' }); // trigger music pause
    showToast('▶ Resumed');
  });
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    ...
    switch (msg.type) {
      ...
      case 'DUAL_RESUME':
        if (v.paused) {
          pausedByUs = false;
          v.play().then(() => removeOverlay()).catch(() => showOverlay(v));
        }
        break;
    }
  });
